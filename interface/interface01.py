...
class MyWindow(Gtk.ApplicationWindow):
	...
	def __init__(self, app):
		...
		self.state = 'copy'
		self.thread = None
		self.callback = Gio.SimpleAction.new('callback_info', None)
		...
		self.grid.attach(self.scrolled_win_select_folder, 0, 7, 2, 1)
		...
		for path_delicate, safe_path in self.safer.safe_dirs.items():
			button = Gtk.RadioButton.new_with_label_from_widget(pre_widget, path_delicate)
        	button.connect("toggled", self.on_button_select_toggled, str(i))
			i += 1
        	select_box.pack_start(button1, False, False, 0)

			#self.listselect.append([path_delicate, first])
			#first = False if first else True
			# safe_path is useless here, path_delicate is the source

		self.grid.attach(select_box, )
		...
		tree_view_column_toggle = Gtk.TreeViewColumn("Activer", cell_renderer_toggle, active=1)
		...
		print(self.scrolled_win_select_file.get_hadjustment())
	...
	def on_cell_toggled_select(self, widget, path):
		...
		print(self.safer.safe_dirs[self.listselect[path][0]]['activate'])
		print(self.safer.safe_dirs)
		# everythings work fine here
