...
class folder_select_dialog(Gtk.Dialog):
	"""docstring for folder_select_diaog"""
	def __init__(self, parent):
		Gtk.Dialog.__init__(self, 'Selection de dossier', parent, 0)
		self.parent = parent
		self.set_modal(True)
		self.set_resizable(False)
		self.set_border_width(10)
		...
		for path_delicate, safe_path in self.parent.safer.safe_dirs.items():
			self.liststore.append([path_delicate, True])
			# safe_path is useless here
			# path_delicate is the source

		treeview = Gtk.TreeView.new_with_model(self.liststore)

		renderer_text = Gtk.CellRendererText()
		column_text = Gtk.TreeViewColumn("Dossier", renderer_text, text=0)
		treeview.append_column(column_text)

		renderer_toggle = Gtk.CellRendererToggle()
		renderer_toggle.connect("toggled", self.on_cell_toggled)
		column_toggle = Gtk.TreeViewColumn("Activer", renderer_toggle, active=1)
...
class compare_dialog(Gtk.Dialog):
	"""docstring for folder_select_diaog"""
	def __init__(self, parent):
		Gtk.Dialog.__init__(self, 'Comparer', parent, 0)
		self.parent = parent
		self.set_modal(True)
		self.set_resizable(False)
		self.set_border_width(10)
